...
Global $TBInstallPath	=	"C:\Users\Administrator\Documents\tbv5332_x64_portable"
...
If $f == 1 Then Filewriteline($TBInstallPath  & "\log\LOG_" & @YEAR & @MON & @MDAY & "_"& $Week[@WDAY - 1]&".txt", "  " & @TAB & "[" & @HOUR & ":" & @MIN & ":" & @SEC & "]" & @TAB & @TAB & "AutoRunTB启动")
If $f == 1 Then Filewriteline($TBInstallPath  & "\log\LOG_" & @YEAR & @MON & @MDAY & "_"& $Week[@WDAY - 1]&".txt", "");另起一行
...
While 1
        ...
        If @WDAY > 1 And @WDAY <= 7 Then	;@WDAY指示当天属该周的第几天,值的范围是 1 到 7,依次表示星期天到星期六.
                ;盘前工作
                If (@MIN > 39 And @MIN <=59 And @SEC <= 59 And (@HOUR = 8 Or @HOUR = 20) And $t <3 And $o ==0) Then;杀进程
                        Kill()
						Sleep(1000)
                ;启动TB
                        TBStar()
...
Func TBStar()
        If Not WinExists ("交易开拓者平台(旗舰版)") Then	;动作前提条件
				...
				Sleep(10000);
                If $f == 1 Then Filewriteline($LogName, "1:" & @TAB & $LogTime & "启动开拓者")
        EndIf
EndFunc
;动作编号2：TB登录
Func TBLogin()
        If WinExists("暂停自动登录交易帐户") = 0 Then 	;动作前提条件
                ...
                If $f == 1 Then Filewriteline($LogName, "2:" & @TAB & $LogTime & "登录开拓者")
...
Func TradeStar()
	  ...
	  ;WinWaitActive("交易开拓者平台(旗舰版) 64位 -","")
        ...
        If ControlListView ( "交易开拓者平台(旗舰版)", "", "SysListView321", "GetItemCount") >= 1	Then
                ...
                If $f == 1 Then Filewriteline($LogName, "3:" & @TAB & $LogTime & "启动交易")
			 EndIf
			 Sleep(3000)
EndFunc
;动作编号4：启动监控
Func MonitorStar()
        ...
        If ControlListView ( "交易开拓者平台(旗舰版)" ,"", "SysListView321", "GetItemCount") >= 1	Then
				...
				ControlClick("自动交易头寸监控器","",2316)
			    Sleep(500)
                If $f == 1 Then Filewriteline($LogName, "4:" & @TAB & $LogTime & "启动监控")
			    $o = 1
			 EndIf
...
Func SaveWorkSpace()
        if WinExists("交易开拓者平台(旗舰版) 64位 -") Then
                ...
                If $f == 1 Then Filewriteline($LogName, "5:" & @TAB & $LogTime & "保存工作区")
        EndIf
EndFunc
;动作编号6：关闭监控
Func MonitorClose()
        if WinExists("自动交易头寸监控器", "") Then
                ...
                If WinExists("确认") Then
                        ...
                        If $f == 1 Then Filewriteline($LogName, "6:" & @TAB & $LogTime & "关闭监控")
                        Sleep(3000)
                EndIf
        EndIf
EndFunc
...
Func TradeStop()
        if WinExists("交易开拓者平台(旗舰版) 64位 -") Then
                ...
                If $f == 1 Then Filewriteline($LogName, "7:" & @TAB & $LogTime & "停止交易")
                Sleep(3000)
        EndIf
EndFunc
;动作编号8：关闭TB
Func TBClose()
		 if WinExists("交易开拓者平台(旗舰版) 64位 -") Then
                ...
                If WinExists("确认", "") Then
                        ...
                        If $f == 1 Then Filewriteline($LogName, "8:" & @TAB & $LogTime & "关闭开拓者")
                        If $f == 1 Then Filewriteline($LogName, "");另起一行
                        Sleep(3000)
						$o = 0
                EndIf
        EndIf
...
Func Expired()
        If (_NowCalcDate() >= $ExpirationDate  ) Then					;动作前提条件
                If $f == 1 Then Filewriteline($LogName, "9:" & @TAB & $LogTime & "程序过期日：" & $ExpirationDate )
...
Func AccountLogin()
        if ControlListView ( "交易开拓者平台(旗舰版)", "", "SysListView321", "GetItemCount") < 1 Then
                ...
                If $f == 1 Then Filewriteline($LogName, "10:" & @TAB & $LogTime & "再次登录")
        EndIf
EndFunc
;动作编号11：登录失败
Func LoginFail()
        If ControlListView ( "交易开拓者平台(旗舰版)", "", "SysListView321", "GetItemCount") < 1 Then
                If $f == 1 Then Filewriteline($LogName, "11:" & @TAB & $LogTime & "柜台关闭")
                TBClose()
        EndIf
EndFunc
;动作编号20：重启系统
Func ShutdownR()
	    If @OSVersion == "WIN_7" Then
			   If $f == 1 Then Filewriteline($LogName, "20:" & @TAB & $LogTime & "周末重启系统")
			   Run("shutdown -r")
			Else
			   If $f == 1 Then Filewriteline($LogName, "20:" & @TAB & $LogTime & "非WIN7系统周末不重启")
		EndIf
EndFunc
;动作编号30：读取交易记录
Func TradeLog()
		 ...
		 if FileExists ($TradeName) Then
			...
			If $TradeLog = -1 Then
			   ;写入日志
			   If $f == 1 Then Filewriteline($LogName, "30:" & @TAB & $LogTime & "错误,无法打开文件." & $TradeName)
			EndIf
			;获得新文件大小
